...
@app.route('/dashboard')    #the dashboard to display all bands
def dashboard():
...
@app.route('/mybands')  #the session[user] who created bands, edit and delete 
def mybands():
    if 'user_id' not in session:
        return redirect('/logout')
    data ={
    ...
    bands = Band.get_user_bands(data)
    return render_template("show_band.html",user = user,all_band = bands)
